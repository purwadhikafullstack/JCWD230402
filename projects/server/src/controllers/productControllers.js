...
module.exports = {
    addProduct: async (req, res, next) => {
        ...
        try {
            ...
            let { name, categoryId, description, variations } = JSON.parse(req.body.data)
            ...
            if (cekProduk.length == 0) { //CASE = produk blm ada sama sekali, buat produk baru di table produk, dan variant di table type
            ...
            } else if (cekProduk.length != 0) { // CASE = kalo produk sudah ada di table produk, dan mau tambah variant
                ...
                for (let i = 0; i < variations.length; i++) {
                    ...
                    if (cektype.length == 0) { // kalo variasi belum ada di database type
                    ...
                    } else { // kalo variasi ternyata sudah ada di database type
                        newArr.push("true")
                        // console.log(`newArr`, newArr);
                    }
                }
                ...
                if (newArr.includes("false")) {
                    ...
                    return res.status(400).send({
                        success: false,
                        message: "produk yang ingin ditambah dan semua variasinya sudah ada di database"
                    })
                }
            }
        } catch (error) {
    ...
    editProduct: async (req, res, next) => {
        try {
            ...
            let { name, categoryId, description, variationsEdit } = JSON.parse(req.body.data)
                    ...
                    let cektype = await model.type.findAll({
                        where: {
                            [sequelize.Op.and]: [
                                ...
                                { colorId: variationsEdit[i].colorId.value || variationsEdit[i].colorId },
                                { memoryId: variationsEdit[i].memoryId.value || variationsEdit[i].memoryId },
    ...
    deleteProduct: async (req, res, next) => {
        try {
            ...
            if (findProduct[0].dataValues.isDisabled == false) {
                ...
                let deleteProduct = await model.product.update({ isDisabled: 1 }, {
                    where: {
                        id: req.params.id
                    }
                })
                ...
                let deleteProduct = await model.product.update({ isDisabled: 0 }, {
                    where: {
                        id: req.params.id
                    }
                })
        ...
        try {
            if (req.query.warehouseId) {
                let get = await model.type.findAll({
                    ...
                    include: [
                        { model: model.status, attributes: ["id", "status"] }
                    ]
                })
    ...
    getColor: async (req, res, next) => {
        try {
            let get = await model.color.findAll({
                attributes: [["id", "value"], ["color", "label"]]
            })
    ...
    getMemory: async (req, res, next) => {
        try {
            let get = await model.memory.findAll({
                attributes: [["id", "value"], ["memory", "label"]]
            })
